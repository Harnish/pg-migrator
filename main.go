...
func (m *Migrator) VerifyRoles() error {
	log.Println("\n=== Verifying Roles ===")

	srcRoles, err := m.GetRoles()
	if err != nil {
		return fmt.Errorf("failed to get source roles for verification: %w", err)
	}

	// Create a map of source roles by name for quick lookup
	srcRoleMap := make(map[string]Role)
	for _, role := range srcRoles {
		srcRoleMap[role.Name] = role
	}

	// Query destination roles (same query as source)
	dstRoles, err := m.queryRoles(m.dstConn)
	if err != nil {
		return fmt.Errorf("failed to get destination roles for verification: %w", err)
	}

	var mismatches []string
	var verified int

	// Compare each destination role with source
	for _, dstRole := range dstRoles {
		srcRole, exists := srcRoleMap[dstRole.Name]
		if !exists {
			// Role exists in destination but not in source (could be expected)
			continue
		}

		// Compare permissions
		if srcRole.Super != dstRole.Super ||
			srcRole.Inherit != dstRole.Inherit ||
			srcRole.CreateRole != dstRole.CreateRole ||
			srcRole.CreateDB != dstRole.CreateDB ||
			srcRole.CanLogin != dstRole.CanLogin ||
			srcRole.Replication != dstRole.Replication ||
			srcRole.ConnLimit != dstRole.ConnLimit {

			mismatch := fmt.Sprintf("Role '%s' has permission mismatches:\n"+
				"  Source: SUPER=%v INHERIT=%v CREATEROLE=%v CREATEDB=%v LOGIN=%v REPLICATION=%v CONNLIMIT=%d\n"+
				"  Dest:   SUPER=%v INHERIT=%v CREATEROLE=%v CREATEDB=%v LOGIN=%v REPLICATION=%v CONNLIMIT=%d",
				dstRole.Name,
				srcRole.Super, srcRole.Inherit, srcRole.CreateRole, srcRole.CreateDB, srcRole.CanLogin, srcRole.Replication, srcRole.ConnLimit,
				dstRole.Super, dstRole.Inherit, dstRole.CreateRole, dstRole.CreateDB, dstRole.CanLogin, dstRole.Replication, dstRole.ConnLimit)
	...
	if len(mismatches) > 0 {
		log.Printf("⨯ Found %d role permission mismatches (out of %d verified):", len(mismatches), verified+len(mismatches))
		for _, mismatch := range mismatches {
			log.Printf("\n%s", mismatch)
		}
		return fmt.Errorf("role verification failed: %d permission mismatches found", len(mismatches))
	}

	log.Printf("✓ Verified %d roles - all permissions match", verified)
	return nil
}

func (m *Migrator) queryRoles(conn *sql.DB) ([]Role, error) {
	query := `
		SELECT 
			rolname, 
			rolsuper, 
